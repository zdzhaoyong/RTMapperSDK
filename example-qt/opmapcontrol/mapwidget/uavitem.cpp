...
UAVItem::UAVItem(MapGraphicItem* map, OPMapWidget* parent, QString uavPic) :
    ...
    autosetreached(true), autosetdistance(5)
{
                        ...
                        const QColor &color)
{
    ...
    if(coord!=position) {
        if(trailtype==UAVTrailType::ByTimeElapsed) {
            if(_timer.elapsed() > trailtime*1000) {
        ...
        if( mapwidget->GCS != 0 ) {
            //verify if the UAV is inside the safety bouble
            if(Distance3D(mapwidget->GCS->Coord(),mapwidget->GCS->Altitude())>mapwidget->GCS->SafeArea()) {
                if(mapwidget->GCS->safe!=false) {
                    mapwidget->GCS->safe=false;
                    mapwidget->GCS->update();
                    emit UAVLeftSafetyBouble(this->coord);
                }
            } else {
                if(mapwidget->GCS->safe!=true) {
                    mapwidget->GCS->safe=true;
                    mapwidget->GCS->update();
                }
            }
        }
